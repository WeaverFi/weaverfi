...
const uiDataProvider: Address = '0x1dDAF95C8f58d1283E9aE5e3C964b575D7cF7aE3';
...
export const getIncentives = async (ibTokens: Record<Address, { aTokenAddress: Address, variableDebtTokenAddress: Address }>, wallet: Address) => {
  if(Object.keys(ibTokens).length > 0) {
    ...
    let rewards = parseInt(await query(chain, incentives, aave.incentivesABI, 'getUserRewards', [tokens, wallet, wavax]));
    if(rewards > 0) {
