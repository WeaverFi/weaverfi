...
const uiDataProvider: Address = '0x7006e5a16E449123a3F26920746d03337ff37340';
...
export const getIncentives = async (ibTokens: Record<Address, { aTokenAddress: Address, variableDebtTokenAddress: Address }>, wallet: Address) => {
  if(Object.keys(ibTokens).length > 0) {
    ...
    let rewards = parseInt(await query(chain, incentives, aave.incentivesABI, 'getUserRewards', [tokens, wallet, wmatic]));
    if(rewards > 0) {
